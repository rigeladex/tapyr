...
class MOM_Sort_Key_Converter (TFL.Meta.Object) :
    """Try to convert the result of MOM.Entity.sort_key () into something
       which can be used for a SQLAlchemy query order_by clause.
    """

    Ignore = set (("relevant_root.type_name", "epk_sig"))

    def __init__ (self) :
        self._Hash = {}
    # end def __init__

    def __call__ (self, sort_key) :
        if sort_key not in self._Hash :
            self._Hash [sort_key] = TFL.Sorted_By \
                (* self._convert_sorted_by (sort_key))
        return self._Hash [sort_key]
    # end def __call__

    def _convert_sorted_by (self, sorted_by) :
        criteria = []
        for c in sorted_by.criteria :
            if c in self.Ignore :
                continue
            if isinstance (c, types.MethodType) :
                criteria.extend \
                    (getattr (self, c.__name__.replace ("__", "_")) (c))
            else :
                criteria.append (c)
        return criteria
    # end def _convert_sorted_by

    def _sort_key (self, criteria) :
        return self._convert_sorted_by (criteria.im_self.sorted_by)
    # end def _sort_key

    def sort_key (self, criteria) :
        if isinstance (criteria.im_self, MOM.Attr.Link_Role) :
            return self._convert_sorted_by \
                (criteria.im_self.role_type.sorted_by)
        return ()
    # end def sort_key

# end class MOM_Sort_Key_Converter
